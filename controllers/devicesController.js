...
const getAllDevices = async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT * FROM devices LIMIT ${limit} OFFSET ${offset}`
    );
    ...
    const devicesWithStatus = devicesresults.map((device) => ({
      ...device,
      status: vehicleIMEIs.includes(device.IMEI)
        ? "Installed"
        : "Not Installed",
    }));
    const installedCount = devicesWithStatus.filter(
      (IMEI) => IMEI.status === "Installed"
    ).length;
    const notInstalledCount = devicesWithStatus.filter(
      (IMEI) => IMEI.status === "Not Installed"
    ).length;
    res.status(200).send({
      success: true,
      installedCount,
