...
const engineOnSendMessage = async (req, res) => {
    try {
        ...
        const trackingResult = await query(smsTrackingQuery, [Vehicle_Label, datePart]);

        if (trackingResult.length > 0 && trackingResult[0].count >= 2) {
            // If already sent 2 messages today, do not send another one
            res.status(429).send({ success: false, message: 'Message limit reached for today.' });
            return;
        }
                    ...
                    axios.request(options)
                        .then(async function (response) {
                            ...
                            if (trackingResult.length > 0) {
                                await query(`UPDATE sms_tracking SET count = count + 1 WHERE Vehicle_Label = ? AND date = ?`, [Vehicle_Label, datePart]);
                            } else {
                                await query(`INSERT INTO sms_tracking (Vehicle_Label, date, count) VALUES (?, ?, 1)`, [Vehicle_Label, datePart]);
                            }
                            res.status(200).send({
                                success: true,
