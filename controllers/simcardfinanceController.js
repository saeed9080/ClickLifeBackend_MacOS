...
const getAllSimCardFinance = async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT * FROM SimCardFinance LIMIT ${limit} OFFSET ${offset}`
    );
    ...
    const vehicleSimNumbers = vehicleSim_Number.map(
      (vehicle) => vehicle.Sim_Number
    );
    const simcardfinanceWithStatus = simcardfinanceresults.map(
      (simcardfinance) => ({
        ...simcardfinance,
        status: vehicleSimNumbers.includes(simcardfinance.sim)
          ? "Installed"
          : "Not Installed",
      })
    );
    const installedCount = simcardfinanceWithStatus.filter(
      (sim) => sim.status === "Installed"
    ).length;
    const notInstalledCount = simcardfinanceWithStatus.filter(
      (sim) => sim.status === "Not Installed"
    ).length;
    res.status(200).send({
      success: true,
      installedCount,
...
const searchController = async (req, res) => {
  try {
    ...
    const vehicleSim_NumberResults = await query(
      "SELECT Sim_Number FROM vehicle"
    );
    const vehicleSimNumbers = vehicleSim_NumberResults.map(
      (vehicle) => vehicle.Sim_Number
    );
